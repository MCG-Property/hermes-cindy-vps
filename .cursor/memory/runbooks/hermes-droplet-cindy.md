...
- **Repo path:** `/root/hermes-agent` (clone `https://github.com/cindyagent01/hermes-agent.git` with submodules).
...
- **Same remote as local clone:** `https://github.com/cindyagent01/hermes-agent.git`, branch **`main`**, tracking **`origin/main`**.
...
- **`hermes <subcommand> [args...] droplet-cindy`** — same SSH host; runs **`hermes`** inside that venv remotely (last word **`droplet-cindy`** is stripped before invoking remote `hermes`). Examples: `hermes onboard droplet-cindy`, `hermes doctor droplet-cindy`, `hermes droplet-cindy` (interactive TUI over SSH when `hermes` has no other args).
